...
function Navbar() {
    ...
    return (
        <nav ref={menuRef} className="bg-white/50 px-3 flex flex-row justify-between items-center relative text-primary">
            <div>
                <Link to={"/"}>
                ...
                {links.map(link => (
                    <li className='w-fit items-center' key={link.name}>
                        <NavLink
                            ...
                            className={({ isActive }) => isActive ? "border-2 border-secondary p-2 rounded-lg hover:bg-highlight" : "p-2 hover:bg-highlight hover:shadow-md rounded-md"}
                        >
                            {link.name}
                        </NavLink>
                    </li>
            ...
            {isMobile && (
                <button
                    onClick={toggleMenu}
                    className="md:hidden w-fit text-3xl text-primary focus:outline-none
            ...
            {/* Mobile menu */}
            <ul className={`
                md:hidden absolute left-0 right-0 bg-highlight/90 shadow-lg
